...
## PR-25 기록
- 변경 요약: 핵심 화면 API 컨트롤러 추가, DB 스모크 테스트 문서 증빙
- 변경 이유: 화면과 CRUD API를 연결하고 PR 정책(코드+스모크+문서)을 충족하기 위함
- 검증 방법: DB 스모크 테스트 수행(테이블 카운트 확인)
- 결과: 두 테넌트 DB에서 초기 데이터 존재 확인

## PR-26 기록
- 변경 요약: CRUD API 입력 검증/감사로그 강화, JSP 폼/버튼 연결, API 스모크 시나리오 문서 보강
- 변경 이유: 화면-API 연결과 기본 검증을 확보해 실제 테스트 흐름을 제공하기 위함
- 검증 방법: DB 스모크 테스트 완료, API 스모크(조회) 수행 완료
- 결과: 화면에서 CRUD 호출 가능, API 조회 응답 success 확인

## PR-27 기록
- 변경 요약: MES Web Service CRUD 스모크 결과 문서 업데이트
...
## PR-29 기록
- 변경 요약: 권한 정책/감사 로그 해시 적용, CRUD 검증/페이징 보강, 공통 버튼 컴포넌트 추가
- 변경 이유: 권한/보안 기준을 서버에서 확정하고 UI 재사용성과 검증 품질을 높이기 위함
- 검증 방법: 빌드 수행, 권한/감사 로그 스모크 수행
- 결과: 관리자/조회 전용 권한 동작 확인, 감사 로그 해시 저장 확인

## PR-30 기록
- 변경 요약: 권한 매트릭스 화면 구현, 조회 조건 날짜 검증 강화
- 변경 이유: 권한 정책을 화면에서 확인 가능하게 하고 잘못된 조회 입력을 차단하기 위함
- 검증 방법: 빌드 수행, 권한 매트릭스 화면/날짜 검증 스모크 수행
- 결과: 권한 화면 접근 제어 확인, 잘못된 날짜 입력 시 fail 응답 확인

## PR-31 기록
- 변경 요약: 권한 저장 기능/DB 권한 테이블 적용, 권한 저장 스모크 증빙
- 변경 이유: 권한 매트릭스를 수정 가능한 형태로 운영하기 위함
- 검증 방법: 빌드 수행, 권한 저장 스모크 수행
- 결과: VIEWER 권한 저장 요청 302 확인

## PR-32 기록
- 변경 요약: 역할 관리 서비스/화면 추가, SYSTEM_ADMIN 권한 보호 강화, 작업/품질/설비 조회 검증 보강
- 변경 이유: 역할 운영을 서버 기준으로 확정하고 입력 오류를 사전에 차단하기 위함
- 검증 방법(명령어):
  - mvn -f mes-web/pom.xml -DskipTests package
  - mvn -f mes-web/pom.xml test
- 결과(PASS 근거):
  - 패키징 빌드 성공
  - 테스트 실행 성공(실행할 테스트 없음)

## PR-33 기록
- 변경 요약: MES Web Service 스모크 결과(역할/권한/CRUD) 문서 반영
- 변경 이유: 최신 스모크 결과를 문서에 고정해 PR 정책(코드+스모크+문서)을 충족하기 위함
- 검증 방법: 서버 기동 후 관리자 로그인 및 역할/권한/CRUD 스모크 수행
- 결과: 관리자 화면 접근 및 역할/권한 보호 정상, 수주/작업/재고/품질/설비 CRUD result=success 확인

## PR-34 기록
- 변경 요약: 역할 체계 문서 정합성 보완(SYSTEM_ADMIN/MANAGER/OPERATOR/VIEWER) 및 역할 설명 주석 보강
- 변경 이유: UI/UX 전면 수정 전에 역할 기준을 코드와 문서에서 일치시키기 위함
- 검증 방법(명령어):
  - mvn -f mes-web/pom.xml test
- 결과: 테스트 실행 성공(실행할 테스트 없음)
